...
public class TransactionServiceTests
{
	...
	[Fact]
	public async Task DeleteAsync_CascadeChangedSyncRecords_PublishesSilentCollectionChange()
	{
		// Arrange
		List<Guid> ids = [Guid.NewGuid(), Guid.NewGuid()];
		_mockRepository.Setup(r => r.DeleteAsync(ids, It.IsAny<CancellationToken>()))
			.ReturnsAsync(new CascadeMutationResult(true, 2));
	...
	[InlineData(0, false)]
	[InlineData(2, true)]
	public async Task RestoreAsync_PublishesOnlyWhenDurableCascadeChangedSyncRecords(int changed, bool publish)
	{
		Guid id = Guid.NewGuid();
		_mockRepository.Setup(r => r.RestoreAsync(id, It.IsAny<CancellationToken>()))
			.ReturnsAsync(new CascadeMutationResult(true, changed));
		if (publish)
		{
			_publisher.Setup(p => p.PublishAsync(It.IsAny<CommittedEntityChange>())).Returns(Task.CompletedTask);
		}

		(await _service.RestoreAsync(id, CancellationToken.None)).Should().BeTrue();

		VerifyPublished(CommittedChangeType.Updated, publish ? Times.Once() : Times.Never());
	}

	[Fact]
	public async Task DeleteAsync_RepositoryFailure_DoesNotPublish()
	{
		_mockRepository.Setup(r => r.DeleteAsync(It.IsAny<List<Guid>>(), It.IsAny<CancellationToken>()))
