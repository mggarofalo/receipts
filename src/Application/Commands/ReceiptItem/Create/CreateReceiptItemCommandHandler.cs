	...
	IReceiptService receiptService) : IRequestHandler<CreateReceiptItemCommand, List<Domain.Core.ReceiptItem>>
{
	public async ValueTask<List<Domain.Core.ReceiptItem>> Handle(CreateReceiptItemCommand request, CancellationToken cancellationToken)
	{
		...
		// Provenance stays separate from caller-writable item data. Persistence resolves the
		// current template under its write guard, rather than stamping a stale domain object.
		return request.ItemTemplateIds.Count == 0
			? await receiptitemService.CreateAsync([.. request.ReceiptItems], request.ReceiptId, cancellationToken)
			: await receiptitemService.CreateAsync([.. request.ReceiptItems], request.ReceiptId, request.ItemTemplateIds, cancellationToken);
	}
}
