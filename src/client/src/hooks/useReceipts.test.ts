...
describe("selected receipt detail", () => {
  it("does not fetch a known selected ID when its hidden parent already supplies context", () => {
    const { result } = renderHook(() => useReceipt("receipt", { enabled: false }), { wrapper: createWrapper() });
    expect(result.current.fetchStatus).toBe("idle");
    expect(client.GET).not.toHaveBeenCalled();
  });

  it("passes cancellation and local presentation while preserving detail shape", async () => {
    const receipt = { id: "receipt", location: "Market", date: "2026-01-15", taxAmount: 1.25 };
    (client.GET as Mock).mockResolvedValue({ data: receipt });
    const { result } = renderHook(() => useReceipt(receipt.id), { wrapper: createWrapper() });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toEqual(receipt);
    expect(client.GET).toHaveBeenCalledWith("/api/receipts/{id}", {
